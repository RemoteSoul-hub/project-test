...
export default function EditPartnerPage({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [partner, locations, plans, osTemplateGroups, brands] = await Promise.all([
          AdminApiService.getPartner(token, partnerId),
        ...
        setFormData({
          ...
          account_manager_name: partnerData.account_manager_name || '',
          account_manager_email: partnerData.account_manager_email || '',
          cc_emails: partnerData.cc_emails || '',
          phone_number: partnerData.phone_number || '',
          vat_number: partnerData.vat_number || '',
          sales_agent: partnerData.sales_agent || '',
          language: partnerData.language || 'en',
          logo_url: partnerData.logo_url || '',
          client_support: Boolean(partnerData.client_support),
          notes: partnerData.notes || '',
          disabled: Boolean(partnerData.disabled),
          partnership_type: partnerData.partnership_type || '',
          emails_disabled: Boolean(partnerData.emails_disabled),
          new_server_bcc_email: partnerData.new_server_bcc_email || '',
          new_application_bcc_email: partnerData.new_application_bcc_email || '',
          api_access: Boolean(partnerData.api_access),
          ip_whitelist: partnerData.ip_whitelist || '',
          hubspot_link: partnerData.hubspot_link || '',
          invoice_emails_enabled: partnerData.invoice_emails_enabled !== false,
          reminder_emails_enabled: partnerData.reminder_emails_enabled !== false,
          credit_amount: partnerData.credit_amount || 0,
          is_auto_terminated: Boolean(partnerData.is_auto_terminated),
          brand_id: partnerData.brand?.id || '',
          username_prefix: partnerData.username_prefix || '',
          location_plans: partnerData.location_plans?.map(lp => ({
            location_id: lp.location.id,
            plan_id: lp.plan.id,
            price: lp.price
          })) || [],
          os_template_group_ids: partnerData.os_template_groups?.map(group => group.id) || []
        });
      } catch (error) {
        console.error('Error fetching data:', error);
        setErrors({ general: error.message || 'Failed to load partner data' });
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await AdminApiService.updatePartner(token, partnerId, payload);
