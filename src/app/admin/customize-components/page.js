      ...
      cell: (value, row) => (
        <div className="flex gap-2">
          <button
            ...
            title={!row.is_available ? 'Component unavailable - out of stock or not in DataPacket' : ''}
          >
...
<div className="flex justify-between items-center mb-6">
  ...
  <div className="flex gap-3">
    <button
      ...
      className={`px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 ${(syncing || resetting) ? 'cursor-not-allowed' : ''}`}
      title="Delete all components from database"
    >
    ...
    <button
      ...
      disabled={syncing || resetting}
      className={`px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 ${(syncing || resetting) ? 'cursor-not-allowed' : ''}`}
    >
