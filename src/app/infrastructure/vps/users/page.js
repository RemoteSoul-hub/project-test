...
export default function VpsUsersPage() {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      // Always fetch main table data when search term changes
      fetchUsers(1, searchTerm, sortField, sortDirection, activeFilters); 
      
      // Also fetch preview results if search term is not empty
      if (searchTerm) {
        searchUsers(searchTerm); 
      } else {
        setSearchResults([]); // Clear preview if search term is empty
      }
    }, 300);

    return () => clearTimeout(delayDebounceFn);
  // Remove currentPage from dependencies as we always fetch page 1 on search change
  ...
  const fetchUsers = async (page, search, sort, direction, filters) => {
    try {
      ...
      console.error('Error fetching VPS users:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const searchUsers = async (search) => {
    try {
      ...
      console.error('Error searching VPS users:', error);
      setSearchResults([]);
  ...
  // Updated formatDate to include time
  const formatDate = (dateString) => {
    if (!dateString) return '';
    const date = new Date(dateString);
    // Example: 01/05/2025 16:27:30
    return `${date.toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })} ${date.toLocaleTimeString('en-GB')}`;
  };
  ...
  const filterFields = [
    // Change 'daterange' to two separate 'date' fields
    {
      name: 'dateFrom', // Corresponds to activeFilters.dateFrom
      label: 'Created From',
      type: 'date'
    },
    {
      name: 'dateTo', // Corresponds to activeFilters.dateTo
      label: 'Created To',
      type: 'date'
    },
    {
      ...
      label: 'Role',
      type: 'select',
      options: roleOptions.map(role => ({ value: role.toLowerCase(), label: role })) // Map roles to options format
    }
  ];

  // Reordered columns and updated date formatting
  const columns = [
    {
      header: 'User',
    ...
    // Moved Date Created to be the second column
    {
      header: 'Date Created',
      accessor: 'created_at',
      sortable: true,
      cell: (value) => formatDate(value), // Use updated formatDate for timestamp
      headerCell: () => (
        <div
          className="flex items-center cursor-pointer"
    ...
    <div className="relative">
      <TableToolbar 
        ...
        search={{
          ...
          renderResult: renderSearchResult, // Use the new renderSearchResult
      ...
      <FilterPanel 
        ...
        // roleOptions={roleOptions} // Remove this, pass filterFields instead
        filterFields={filterFields} // Pass the defined filter fields
        buttonRef={filterButtonRef}
      />
